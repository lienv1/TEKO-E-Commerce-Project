...
public interface ProductRepo extends JpaRepository<Product, Long>, JpaSpecificationExecutor<Product> {
    ...
    @Query("SELECT DISTINCT p.brand FROM Product p WHERE EXISTS (SELECT f FROM Favorite f WHERE f.product = p AND f.user.username = :username)")
    Set<String> findFavoriteBrands(String username);
    @Query("SELECT DISTINCT p.origin FROM Product p WHERE EXISTS (SELECT f FROM Favorite f WHERE f.product = p AND f.user.username = :username)")
